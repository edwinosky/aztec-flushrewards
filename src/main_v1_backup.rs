...
async fn main() -> Result<()> {
    ...
    let slot_duration = rollup_reader.getSlotDuration().call().await?._0.to_string().parse::<u64>()?;
    println!("⏱️ Slot Duration: {}s", slot_duration);
    
    let current_slot = rollup_reader.getCurrentSlot().call().await?._0.to_string().parse::<u64>()?;
                ...
                if !nonce_fetched {
                    match read_provider.get_transaction_count(my_address).await {
                        Ok(n) => {
                            current_nonce = n;
                            nonce_fetched = true;
                            // LOCK THE EPOCH: Prevent fetching/resetting nonce again for this epoch
